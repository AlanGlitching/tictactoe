class TicTacToeMultiplayerClient {
    ...
    initializeElements() {
        ...
        this.slotScreen = document.getElementById('slot-screen');
        ...
        this.choiceSlotBtn = document.getElementById('choice-slot-btn'); // Slot Machine
    ...
    attachEventListeners() {
        ...
        this.choiceSlotBtn.addEventListener('click', () => this.showSlotScreen());
    ...
    showSlotScreen() {
        this.hideAllScreens();
        this.slotScreen.classList.remove('hidden');
        this.initializeSlotGame();
    }
    ...
    hideAllScreens() {
        [this.welcomeScreen, this.choiceScreen, this.tictactoeMenuScreen, this.createScreen, this.joinScreen, this.vsAiScreen, this.quickmatchScreen, this.guessingScreen, this.memoryScreen, this.slotScreen, this.gameScreen].forEach(screen => {
    ...
    initializeMemoryGame() {
        ...
        initializeSlotGame() {
            // Always recreate the slot machine to ensure fresh state
            console.log('Initializing slot machine...');
            window.slotMachine = new SlotMachine();
            
            // Force immediate refresh
            if (window.slotMachine) {
                console.log('Refreshing slot machine...');
                window.slotMachine.forceRefreshDisplay();
                
                // Additional checks to ensure no blank reels
                setTimeout(() => {
                    window.slotMachine.symbolContainers.forEach((container, index) => {
                        if (container && container.children.length === 0) {
                            console.log(`Detected blank reel ${index + 1}, fixing...`);
                            window.slotMachine.initializeSingleReel(container, index);
                        }
                    });
                }, 200);
                ...
                setTimeout(() => {
                    window.slotMachine.symbolContainers.forEach((container, index) => {
                        if (container && container.children.length === 0) {
                            console.log(`Second check - blank reel ${index + 1}, forcing fix...`);
                            window.slotMachine.initializeSingleReel(container, index);
                        }
                    });
                }, 500);
    ...
    constructor() {
            ...
            // Symbols with different probabilities (more common symbols appear more often)
            this.symbols = [
                '🍎', '🍎', '🍎', '🍎', '🍎', '🍎', '🍎', '🍎', '🍎', '🍎', // 10 apples (most common)
                '🍋', '🍋', '🍋', '🍋', '🍋', '🍋', '🍋', '🍋', '🍋', '🍋', // 10 lemons
                '🍊', '🍊', '🍊', '🍊', '🍊', '🍊', '🍊', '🍊', '🍊', '🍊', // 10 oranges
                '🍇', '🍇', '🍇', '🍇', '🍇', '🍇', '🍇', '🍇', '🍇', '🍇', // 10 grapes
                '🍒', '🍒', '🍒', '🍒', '🍒', '🍒', '🍒', '🍒', '🍒', '🍒', // 10 cherries
                '💎', // 1 diamond (very rare - 1/51 chance)
                '⭐', // 1 star (very rare - 1/51 chance)
                '7️⃣', // 1 seven (extremely rare - 1/51 chance)
                '🎰' // 1 wild (extremely rare - 1/51 chance)
            ];
            
            // Payout table (reduced multipliers)
            this.payouts = {
                '7️⃣7️⃣7️⃣': 50,     // 50x - Reduced from 777x
                '💎💎💎': 20,      // 20x - Reduced from 100x
                '🍎🍎🍎': 10,      // 10x - Reduced from 50x
                '🍊🍊🍊': 8,       // 8x - Reduced from 25x
                '🍋🍋🍋': 6,       // 6x - Reduced from 15x
                '🍇🍇🍇': 4,       // 4x - Reduced from 10x
                '🍒🍒🍒': 3,       // 3x - Reduced from 8x
                '⭐⭐⭐': 25,       // 25x - Reduced from 200x
                '🎰🎰🎰': 100     // 100x - Reduced from 1000x
            };
            ...
            this.symbolContainers = [
                document.getElementById('slot-symbols1'),
                document.getElementById('slot-symbols2'),
                document.getElementById('slot-symbols3')
            ];
        ...
        initializeReels() {
            ...
            this.symbolContainers.forEach((container, index) => {
                console.log(`Initializing reel ${index + 1}:`, container);
                
                // Clear existing content
                container.innerHTML = '';
                
                // Create 50 symbols for smoother scrolling
                for (let i = 0; i < 50; i++) {
                    const symbol = document.createElement('div');
                    symbol.className = 'symbol';
                    symbol.textContent = this.symbols[Math.floor(Math.random() * this.symbols.length)];
                    
                    // Set all styles inline with !important
                    symbol.style.cssText = `
                        ...
                        border-bottom: 1px solid rgba(243, 156, 18, 0.2) !important;
                ...
                console.log(`Created ${container.children.length} symbols for reel ${index + 1}`);
                
                // Set container styles with !important
                ...
                container.classList.remove('reel-spinning', 'reel-stopping');
                
                // Force a reflow to ensure the symbols are rendered
                container.offsetHeight;
                
                // Triple check that symbols are visible
                if (container.children.length === 0) {
                    console.error(`No symbols created for reel ${index + 1}!`);
                    // Try to recreate symbols
                    for (let i = 0; i < 50; i++) {
                        const symbol = document.createElement('div');
                        symbol.className = 'symbol';
                        symbol.textContent = this.symbols[Math.floor(Math.random() * this.symbols.length)];
                    ...
                    console.log(`Recreated ${container.children.length} symbols for reel ${index + 1}`);
                }
                
                // Final check - if still empty, force create at least one symbol
                if (container.children.length === 0) {
                    console.error(`CRITICAL: Still no symbols for reel ${index + 1}! Forcing creation...`);
                    const emergencySymbol = document.createElement('div');
                    emergencySymbol.className = 'symbol';
                    emergencySymbol.textContent = '🍎'; // Fallback symbol
                    emergencySymbol.style.cssText = `
                        height: 60px !important;
                        width: 100% !important;
                        display: flex !important;
                        align-items: center !important;
                        ...
                        opacity: 1 !important;
                    `;
                    container.appendChild(emergencySymbol);
                    console.log(`Emergency symbol created for reel ${index + 1}`);
                }
            });
        ...
        forceRefreshDisplay() {
            ...
            setTimeout(() => {
                this.symbolContainers.forEach((container, index) => {
                    if (container && container.children.length === 0) {
                        console.log(`Force fixing empty reel ${index + 1}`);
        ...
        initializeSingleReel(container, index) {
            console.log(`Force initializing single reel ${index + 1}`);
            container.innerHTML = '';
            ...
            container.offsetHeight; // Force reflow
            console.log(`Fixed reel ${index + 1} with ${container.children.length} symbols`);
        }
        ...
        fixDisplay() {
            ...
            this.initializeReels();
            this.showMessage('Display fixed!', 'win');
        }
        
        setupEventListeners() {
            document.getElementById('slot-spin-btn').addEventListener('click', () => this.spin());
            document.getElementById('slot-reset-btn').addEventListener('click', () => this.resetGame());
            document.getElementById('slot-back-btn').addEventListener('click', () => this.backToGames());
            document.getElementById('slot-custom-amount').addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    this.addCustomCredits();
                }
            });
        }
        ...
        spin() {
            if (this.isSpinning) {
                this.showMessage('Already spinning!', 'lose');
                return;
            }
            
            if (this.credits < this.bet) {
                this.showMessage('Not enough credits! Add some credits first.', 'lose');
                return;
            }
            ...
            // Also add spinning class to symbol containers
            this.symbolContainers.forEach((container, index) => {
                console.log(`Adding spinning class to container ${index + 1}`);
                container.classList.add('reel-spinning');
            });
            
            // Stop reels at different times for suspense
            setTimeout(() => this.stopReel(0), 800);
            setTimeout(() => this.stopReel(1), 1200);
        ...
        checkResults() {
            const allRows = this.getAllVisibleSymbols();
            console.log('All visible rows:', allRows);
            
            let totalWin = 0;
            let allWinningCombinations = [];
            
            // Check each payline (row) for wins
            allRows.forEach((row, rowIndex) => {
                const rowWins = this.checkPayline(row, rowIndex);
                rowWins.forEach(win => {
                    totalWin += win.payout;
                    allWinningCombinations.push(win);
                });
            });
            
            // Check vertical columns for wins
            for (let col = 0; col < 3; col++) {
                const column = allRows.map(row => row[col]);
                const columnWins = this.checkPayline(column, col + 3); // Use col + 3 to distinguish from rows
                columnWins.forEach(win => {
                    totalWin += win.payout;
                    allWinningCombinations.push(win);
                });
            }
            ...
            if (totalWin > 0) {
                ...
                this.highlightWinningSymbols(allWinningCombinations);
                this.showWinMessage(totalWin, allWinningCombinations);
            } else {
        ...
        checkPayline(payline, paylineIndex) {
            const winningCombinations = [];
            
            // 1. Check for exact 3-of-a-kind matches (highest priority)
            if (payline[0] === payline[1] && payline[1] === payline[2]) {
                const combination = payline[0] + payline[1] + payline[2];
                ...
                if (multiplier > 0) {
                    ...
                    winningCombinations.push({
                        ...
                        winningPositions: [0, 1, 2] // All three positions
                    });
                }
            }
            
            // 2. Check for 2-of-a-kind matches (only if no 3-of-a-kind)
            else if (payline[0] === payline[1] || payline[1] === payline[2] || payline[0] === payline[2]) {
                const payout = this.bet * 2; // 2x multiplier for any 2 match
                
                // Determine which positions are winning
                let winningPositions = [];
                if (payline[0] === payline[1]) {
                    winningPositions = [0, 1]; // First two match
                } else if (payline[1] === payline[2]) {
                    winningPositions = [1, 2]; // Last two match
                } else if (payline[0] === payline[2]) {
                    winningPositions = [0, 2]; // First and last match
                }
            ...
            // 3. Check for wild symbol combinations
            const wildCount = payline.filter(symbol => symbol === '🎰').length;
            if (wildCount > 0) {
                const nonWildSymbols = payline.filter(symbol => symbol !== '🎰');
                
                // If we have 2 non-wild symbols that are the same, wild can complete the set
                if (nonWildSymbols.length === 2) {
                    const uniqueSymbols = [...new Set(nonWildSymbols)];
                    if (uniqueSymbols.length === 1) {
                        const symbol = uniqueSymbols[0];
                        const combination = symbol + symbol + symbol;
                        const multiplier = this.payouts[combination] || 0;
                        
                        if (multiplier > 0) {
                            const payout = this.bet * multiplier;
                            winningCombinations.push({
                                type: 'wild-three-of-a-kind',
                                symbols: payline,
                                payout: payout,
                                payline: paylineIndex,
                                multiplier: multiplier
                ...
                // If we have 1 non-wild symbol and 1 wild, check for 2-of-a-kind
                else if (nonWildSymbols.length === 1 && wildCount === 1) {
                    const payout = this.bet * 2;
                    winningCombinations.push({
                        type: 'wild-two-of-a-kind',
                        symbols: payline,
                        payout: payout,
                        payline: paylineIndex,
                        multiplier: 2
        ...
        getAllVisibleSymbols() {
            ...
            for (let row = 0; row < 3; row++) {
                ...
                this.symbolContainers.forEach((container, containerIndex) => {
                    ...
                    // Calculate which symbol is at the specific visible row position
                    // We want the 3 visible symbols: top (row 0), middle (row 1), bottom (row 2)
            ...
            console.log('Visible symbols in 3 rows:', allRows);
            return allRows;
        }
        
        getVisibleSymbols() {
            // This is kept for backward compatibility, returns middle row
            const allRows = this.getAllVisibleSymbols();
            return allRows[1]; // Return middle row
        }
        ...
        highlightWinningSymbols(winningCombinations) {
            ...
            // Highlight winning symbols in each payline
            winningCombinations.forEach(win => {
                const payline = win.payline;
                const symbols = win.symbols;
                const winningPositions = win.winningPositions || [0, 1, 2]; // Default to all positions
                
                // Check if this is a vertical column (payline >= 3)
                if (payline >= 3) {
                    // Vertical column win - highlight all three visible symbols in the column
                    const columnIndex = payline - 3;
                    this.symbolContainers.forEach((container, reelIndex) => {
                        if (reelIndex === columnIndex) {
                            const containerSymbols = container.querySelectorAll('.symbol');
                            const currentTransform = container.style.transform;
                            const currentY = parseFloat(currentTransform.match(/translateY\(([^)]+)\)/)?.[1] || '0');
                            
                            // Highlight all three visible symbols in this column
                            for (let row = 0; row < 3; row++) {
                                const symbolIndex = Math.floor(Math.abs(currentY) / 60) + row;
                                const actualIndex = symbolIndex % containerSymbols.length;
                                
                                if (containerSymbols[actualIndex]) {
                                    containerSymbols[actualIndex].classList.add('winning');
                                }
                            }
                        }
                    });
                } else {
                    // Horizontal row win - highlight only the winning positions
                    this.symbolContainers.forEach((container, reelIndex) => {
                        const containerSymbols = container.querySelectorAll('.symbol');
                        const currentTransform = container.style.transform;
                        const currentY = parseFloat(currentTransform.match(/translateY\(([^)]+)\)/)?.[1] || '0');
                        
                        // Only highlight the specific winning positions
                        winningPositions.forEach(position => {
                            const symbolIndex = Math.floor(Math.abs(currentY) / 60) + payline;
                            const actualIndex = symbolIndex % containerSymbols.length;
        ...
        showWinMessage(totalWin, winningCombinations) {
            let message = `🎉 You won ${totalWin} credits!`;
            
            if (winningCombinations.length > 0) {
                const paylineNames = ['Top Row', 'Middle Row', 'Bottom Row', 'Left Column', 'Middle Column', 'Right Column'];
                const winDetails = winningCombinations.map(win => {
                    const paylineName = paylineNames[win.payline] || `Payline ${win.payline + 1}`;
                    const symbolDisplay = win.symbols.join('');
                    return `${paylineName}: ${symbolDisplay} (${win.multiplier}x)`;
                });
        ...
        updateDisplay() {
            document.getElementById('slot-credits').textContent = this.credits;
            document.getElementById('slot-current-bet').textContent = this.bet;
            document.getElementById('slot-total-won').textContent = this.totalWon;
            document.getElementById('slot-games-played').textContent = this.gamesPlayed;
        }
    ...
    addCredits(amount) {
            this.credits += amount;
        this.updateDisplay();
            this.showMessage(`Added ${amount} credits!`, 'win');
        }
        
        addCustomCredits() {
            const input = document.getElementById('slot-custom-amount');
            const amount = parseInt(input.value);
            if (amount && amount > 0 && amount <= 10000) {
                this.addCredits(amount);
                input.value = '';
            } else {
                this.showMessage('Please enter a valid amount (1-10000)', 'lose');
            }
    }
    ...
    resetCredits() {
            this.credits = 0;
        this.updateDisplay();
            this.showMessage('Credits reset!', 'lose');
        }
        ...
        this.isSpinning = false;
            ...
            this.initializeReels();
        this.updateDisplay();
            this.showMessage('Game reset!', 'win');
        }
        
        backToGames() {
            if (window.gameClient) {
                window.gameClient.hideAllScreens();
                window.gameClient.showChoiceScreen();
            }
        }
    }

    // Global functions for slot machine HTML buttons
function changeSlotBet(amount) {
        if (window.slotMachine) {
            window.slotMachine.changeBet(amount);
        }
    }
    
    function addSlotCredits(amount) {
        if (window.slotMachine) {
            window.slotMachine.addCredits(amount);
        }
    }
    
    function addSlotCustomCredits() {
        if (window.slotMachine) {
            window.slotMachine.addCustomCredits();
        }
    }
    
    function resetSlotCredits() {
        if (window.slotMachine) {
            window.slotMachine.resetCredits();
        }
    }
    
    function resetSlotGame() {
        if (window.slotMachine) {
            window.slotMachine.resetGame();
        }
    }
    
    // Force initialize slot machine on page load
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setTimeout(() => {
            if (window.slotMachine) {
                console.log('Delayed refresh of slot machine...');
                window.slotMachine.forceRefreshDisplay();
            }
        }, 500);
    }); 